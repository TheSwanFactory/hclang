...
**Date**: 2025-12-14
**Related**: [08-maml-redesign.md](08-maml-redesign.md)

## Problem

Changed MAML from `FrameParam.there()` (`^`) to `FrameArg.level(2)` (`__`), but tests still fail with `.tag` not found.
...
The problem isn't about `^` vs `__`. The problem is about where `.tag` is looked up.
...
`FrameSymbol("tag")` is trying to look up `.tag` in the evaluation context. But when the `HeadBlock` closure is called during `&&` iteration:
...
The `HeadBlock` needs to be defined in a context where `tag` is accessible. Currently:
...
Instead of `FrameSymbol("tag")`, use a reference that will look up in maml's metadata.
