...
def check_results(results: dict[str, object]) -> None:
    ...
    # The recorded Python version identifies the run that produced the checked-in
    # artifact.  It is provenance, not an arithmetic result, so preserve it while
    # comparing results generated by another supported Python 3 runtime.
...
def main() -> None:
    ...
    if arguments not in ([], ["--check"]):
        raise SystemExit("usage: python3 experiments.py [--check]")
