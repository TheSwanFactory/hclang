...
This file is itself executable, and the test suite runs it and checks every
answer below, so the model it teaches cannot drift from what the interpreter
does.
...
## One detail that looks like an exception

A declaration prints once, from the property plane it wrote — but only the value
